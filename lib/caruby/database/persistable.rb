...
module CaRuby
  ...
  module Persistable
    ...
    def database
      raise Jinx::ValidationError.new("#{self} database is missing")
    end
    ...
    def take_snapshot
      ss = @snapshot.dup if @snapshot
      @snapshot = value_hash(self.class.updatable_attributes)
      self.class.updatable_attributes.each_property do |prop|
        if prop.domain? and prop.collection? then
          v = @snapshot[prop.attribute] || next
          @snapshot[prop.attribute] = Set.new(v)
        end
      end
    end
    ...
    def merge_into_snapshot(other)
      if @snapshot.nil? then
        raise Jinx::ValidationError.new("Cannot merge #{other.qp} content into #{qp} snapshot, since #{qp} does not have a snapshot.")
      end
    ...
    def add_lazy_loader(loader, attributes=nil)
      ...
      if identifier.nil? then raise Jinx::ValidationError.new("Cannot add lazy loader to an unfetched domain object: #{self}") end
    ...
    # @return [<Symbol>] the domain attributes to fetch
    ...
    def load_reference(attribute)
      ...
      # If the lazy loader is disabled, then bypass the singleton method and call the class instance method.
      ...
      if pa.dependent? then
        ...
        oattr = pa.inverse
        if oattr then
          # Update the dependent snapshot with the owner, since the owner snapshot is taken when fetched but
          # the owner might be set when the fetched dependent is merged into the owner dependent attribute. 
